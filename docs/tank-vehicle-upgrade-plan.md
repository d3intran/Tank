# 坦克载具升级计划：手写地形跟随 → UE5 Chaos 载具

> 起因：M4c 三轮修复（第七~九批）后"上坡还是卡卡的"，且边缘/骑棱姿态仍只能靠夹角度硬压。
> 玩家拍板："直接升级到 UE5 的标准载具。"
> 状态：**计划中，未开工**。开工前先提交了当前版本（commit `bc8cd98`）。

## 0. 一句话结论

换 Chaos 载具不是"把组件挂上去"，而是**四步工程**，其中第一步（把坦克做成骨骼网格）是硬前提、
第三步（联机移动同步重做）是最大的返工。建议先做 0+1 阶段的单人原型验证手感，再决定是否动联机。
...
- [ ] 上坡：逐帧采样 pitch 连续、速度无台阶（对比现在 `GroundAlignSpeed=18` 的跟随延迟）
- [ ] 跨接：坡底/坡顶过渡四角 gap 不再有 10~50cm 的台阶
- [ ] 边缘：压出掩体边缘会滑/会翻，而不是停在半空（现在的"骑棱冻结"彻底消失）
- [ ] 倒车下坡：不浮空、不落体（第七批已达标，升级后不得退化）
- [ ] 三开联机：位置/姿态/炮塔/命中一致；`Net PktLag=100` 下不炸
- [ ] 帧率：PIE 三开 ≥ 60fps
