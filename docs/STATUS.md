...
| 整车缩放 1/2 | 碰撞盒 760×350×236 → **380×175×118**（实测三世界一致）；摄像机臂长跟缩 |
| 头顶名牌 | 名牌（玩家 ID）+ 血条，遮挡/距离剔除沿用；截图确认 |
| 掩体坡道 ×6 | 6 块掩体各一道 40° 坡，坡底按实测地面（road 顶面）下沉 20cm 消除接缝坎；实测坡上 pitch=40 与坡度一致、坠落无伤害 |
| 出生朝向 | 8 个出生点 yaw 修正为「朝圆心」（此前被 Rotator 位置传参坑成 pitch） |
